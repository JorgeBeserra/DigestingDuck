...
namespace DigestingDuck
{
    public class EntradaDigital //: SampleRunner
    {
        ...
        public EntradaDigital(IProgress<string> progress, IqOptionApiDotNetClient api, ActivePair activePair, long positionIdOrigem, OrderDirection instrumentDir, DigitalOptionsExpiryDuration expirationType, Double amountEnrolled, long traderIdCopia)
        {
            IqClientApi = api;
        ...
        public async Task RunSample()
        {
            ...
            var position = await IqClientApi.PlaceDigitalOptions(requestId, activePair, InstrumentDir, ExpirationType, AmountEnrolled);

            Debug.WriteLine("Enviando Entrada - pt2");
            progresso.Report("----------- [Sua Entrada] ----------");
            progresso.Report("Ativo: " + activePair);
            ...
            try
            {
                ...
                db1.AbrirDB();
                var trader = db1.realm.All<Trader>().First(d => d.UserId == TraderIdCopia);

                db1.realm.Write(() =>
                {
                    var a = new TraderCopia();
                    a.Id = requestId;
