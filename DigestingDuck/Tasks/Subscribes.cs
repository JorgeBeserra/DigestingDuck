                ...
                if (IqClientApi.IsConnected)
                {
                    ...
                    IqClientApi.WsClient.PositionChangedObservable().Subscribe(x =>
                    {
                        if(x.Status == "closed")
                        {
                            ...
                            db2.AbrirDB();
                            var tradersCopia = db2.realm.All<TradersCopias>().Where(q => q.PosicaoIdNovo == x.PortfolioChangedEventInfo.OrderIds[0]).First();
                            db2.realm.Write(() =>
                            {
                    ...
                    var aDigital = database.realm.All<AtivosStatus>().Where(x => x.Aberto == true && x.Alvo == true);
                    foreach (var a in aDigital)
                    {

                        var requestId = Guid.NewGuid().ToString().Replace("-", string.Empty);
                        IqClientApi.SubscribeLiveDeal(requestId, Message, (ActivePair)Convert.ToInt32(a.Ativo.Id), DigitalOptionsExpiryType.PT1M);
                        requestId = Guid.NewGuid().ToString().Replace("-", string.Empty);
                        IqClientApi.SubscribeLiveDeal(requestId, Message, (ActivePair)Convert.ToInt32(a.Ativo.Id), DigitalOptionsExpiryType.PT5M);
                        requestId = Guid.NewGuid().ToString().Replace("-", string.Empty);
                        IqClientApi.SubscribeLiveDeal(requestId, Message, (ActivePair)Convert.ToInt32(a.Ativo.Id), DigitalOptionsExpiryType.PT15M);
                    }
                    ...
                    IqClientApi.WsClient.LiveDealObservable().Subscribe(x => {
                        ...
                        try
                        {
                            var trad = db.realm.All<Traders>().Where(q => q.Alvo == true && q.UserId == x.UserId);                          

                            var config = db.realm.All<UsuariosConfiguracao>().First();

                            TimeSpan difference;
                            difference = x.CreatedAt.Value.Subtract(DateTimeOffset.Now);
                            int segundos = difference.Seconds * -1;

                            var posicaoJaExiste = db.realm.All<TradersCopias>().Where(q => q.PosicaoIdOrigem == x.PositionId);
